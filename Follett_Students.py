...
with oracledb.connect(user=un, password=pw, dsn=cs) as con:
    with con.cursor() as cur:  # start an entry cursor
        with open('student_log.txt', 'w') as outputLog:
                                ...
                                if not str(entry[1]) in badnames and not str(entry[2]) in badnames:
                                    ...
                                    if (schoolID not in badschools):  
                                        if (grade == '6' or grade == '7' or grade == '8'): #if they are a middle schooler, we want to get their study hall and put them in a separate output file
                                            ...
                                            print(f'{idNum},{lastName},{firstName},{middleName},{gradYear},{birthday},{gender},"{homeroom}",{grade},{homephone},{schoolID},{status},"{address}",{city},{state},{zipcode},"{guardianEmail}",{email},{patronType},"{studyhall}"',file=outputMiddle) # outputs to the middle school patron file
                                        else:
                                            print(f'{idNum},{lastName},{firstName},{middleName},{gradYear},{birthday},{gender},"{homeroom}",{grade},{homephone},{schoolID},{status},"{address}",{city},{state},{zipcode},"{guardianEmail}",{email},{patronType}',file=outputfile) # outputs to the normal patron file

                            except Exception as err: #catches errors on the overall individual student
                                print('ERROR on user ' + str(entry[0]) + ': ' + str(err))
                                print('ERROR on user ' + str(entry[0]) + ': ' + str(err), file=outputLog)

                    except Exception as er:
                        print(f'ERROR during PowerSchool query and file creation process: {er}')
                        print(f'ERROR during PowerSchool query and file creation process: {er}', file=outputLog)
            try:
                with pysftp.Connection(sftpHOST, username=sftpUN, password=sftpPW, cnopts=cnopts) as sftp:
                    ...
                    print(f'SFTP connection established on {sftpHOST}', file=outputLog)
                ...
                print(f'ERROR during SFTP conneciton and upload: {er}', file=outputLog)
