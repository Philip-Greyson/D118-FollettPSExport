...
with oracledb.connect(user=un, password=pw, dsn=cs) as con:#  create the connecton to the database
    with con.cursor() as cur:  # start an entry cursor
        with open ('staff_log.txt', 'w') as outputLog:
                    ...
                    for entrytuple in rows: # go through each entry (which is a tuple) in rows. Each entrytuple is a single employee's data
                        try:
                            ...
                            if not entry[2] in badnames and not entry[3] in badnames: # check first and last name against array of bad names, only print if both come back not in it
                                ...
                                password = username # they dont actually sign in with the password since it is google SSO, but apparently it needs to be populated?
                                ...
                                if (schoolID == "0" or schoolID == "131" or schoolID == "132" or schoolID == "133" or schoolID == "134" or schoolID == "135"):
                                    schoolID = "5"
                                print(f'{schoolID},{barcode},{districtID},{lastName},{firstName},{gender},{employeeType},{username},{password},{email},{status},{email},{middleName},{staffStatus},{mainSchool},"{lastFirst}"', file=outputfile)
                        except Exception as err:
                            print('ERROR on user ' + str(entrytuple[0]) + ': ' + str(err))
                            print('ERROR on user ' + str(entrytuple[0]) + ': ' + str(err), file=outputLog)
            ...
            # after the staff file is generated and closed, connect via SFTP to the follett server and upload them to the correct directory
            try:
